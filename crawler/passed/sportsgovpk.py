...
class SportsgovpkSpider(BaseSpider):
    name = 'sportsgovpk'
    website_id = 2106
    language_id = 1866
    is_http = 1
    start_urls = ['http://www.sports.gov.pk/LatestNews']
    ...
    def parse(self, response):
        ...
        if self.time is None or DateUtil.formate_time2time_stamp(last_time) >= self.time:
            for i in block:
                ...
                yield Request('http://www.sports.gov.pk' + i.select_one('a')['href'], callback=self.parse_item, meta=response.meta)
    ...
    def parse_item(self, response):
        ...
        item['images'] = ['http://www.sports.gov.pk' + img.get('src') for img in soup.select('.col-sm-12 img')]
