...
class MohrgovpkSpider(BaseSpider):
    name = 'mohrgovpk'
    website_id = 2112
    language_id = 1866
    start_urls = ['https://mohr.gov.pk/LatestNews']
    ...
    def parse(self, response):
        ...
        if self.time is None or DateUtil.formate_time2time_stamp(last_time) >= self.time:
            for i in block:
                ...
                yield Request('https://mohr.gov.pk' + i.select_one('a')['href'], callback=self.parse_item, meta=response.meta)
    ...
    def parse_item(self, response):
        ...
        item['images'] = ['https://mohr.gov.pk' + img.get('src') for img in soup.select('.col-sm-12 img')]
