...
all_common_sql = "SELECT website.id,url,website.c_name,country.`name`,`language`.c_name,remark,developer,CAST(website.start_time AS CHAR) FROM website LEFT LEFT JOIN country ON website.country_id = country.id LEFT LEFT JOIN `language` ON website.lan_id = `language`.id WHERE website.id = {}"
# 时间
